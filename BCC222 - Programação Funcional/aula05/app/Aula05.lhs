...
area (Triangle (Point x1 y1) (Point x2 y2) (Point x3 y3)) =  abs(((x1 * y2 * 1) + (y1 * 1 * x3) + (1 * x2 * y3) - (y1 * x2 * 1) - (x1 * 1 * y3) - (1 * y2 * x3))/2) -- Área do triângulo por determinante
\end{code}
...
\begin{code}
nroElementos :: IntTree -> Int
nroElementos ILeaf = 0
nroElementos (INode _ l r) = 1 + nroElementos (l) + nroElementos (r)
\end{code}
...
intListToTree (ICons x xs) = (INode x l r) intListToTree xs
\end{code}
...
\begin{code}
listClientesOfertas :: [Client] -> [Client]
listClientesOfertas a = [x <- xs | Customer _ _ true]
\end{code}