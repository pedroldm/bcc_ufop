...
\begin{code}
mapLComprehensions :: (a -> b) -> [a] -> [b]
mapLComprehensions _ [] = []
mapLComprehensions f (x:xs) = [f x | x <- xs]
\end{code}
...
evenList :: [Int] -> [Bool]
evenList [] = []
evenList (x : xs) = if (even x)
                    then True : evenList xs
                    else False : evenList xs
...
\begin{code}
filterLComprehensions :: (a -> Bool) -> [a] -> [a]
filterLComprehensions _ [] = []
filterLComprehensions f (x : xs) = [x | x <- xs, f x]
\end{code}
...
\begin{code}
intervalosFilter :: [Int] -> Int -> Int -> [Int]
intervalosFilter [] _ _ = []
intervalosFilter (x : xs) a b = filter (\x -> (x >= a && x <= b)) xs

intervalosRecursao :: [Int] -> Int -> Int -> [Int]
intervalosRecursao [] _ _ = []
intervalosRecursao (x : xs) a b = if (x >= a && x <= b)
                                  then x : intervalosRecursao xs a b
                                  else intervalosRecursao xs a b
\end{code}
...
sumQuadrados [] = 0
sumQuadrados (x : xs) = if (odd x)
                        then (x * x) + sumQuadrados xs
                        else sumQuadrados xs
\end{code}
...
\begin{code}
sumQuadPFS :: [Int] -> Int
sumQuadPFS = sum . map (x * x) . filter odd
\end{code}
...
takeWhileFoldr p xs = foldr step []
    where
...
allFoldr p xs = foldr step []
  where
    step x ac = if p x then True : ac else False : ac
\end{code}
