...
\begin{code}
nrosPerf :: [Int] -> [Int]
nrosPerf xs = [x | x <- xs, sum(init(factors(x))) == x]
\end{code}
...
\begin{code}
primes :: [Int] -> [Int]
primes xs = [x | x <- xs, length(factors(x)) == 2]
\end{code}
...
\begin{code}
vetNorm :: (Float, Float) -> (Float, Float)
vetNorm (x, y) 
    = (c1, c2)
        where
            v = sqrt (x * x + y * y)
...
\begin{code}
menorOuIgual2CP :: [a] -> Bool
menorOuIgual2CP [] = True
menorOuIgual2CP [_] = True
menorOuIgual2CP [_, _] = True
menorOuIgual2CP _ = False

menorOuIgual2Length :: [Int] -> Bool
menorOuIgual2Length n = (length n <= 2)
\end{code}