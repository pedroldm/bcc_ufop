...
takeList a [] = [];
takeList a (x : xs) = if (0 >= a)
                      then [];
                      else x : takeList (a - 1) xs
\end{code}
...
dropList 0 xs = xs
dropList a [] = []
dropList a (x : xs) = dropList (a - 1) xs
\end{code}
...
\begin{code}
removeAllRecursiv :: Int -> [Int] -> [Int]
removeAllRecursiv a [] = []
removeAllRecursiv a (x : xs) = if ( a == x )
                               then x : removeAllRecursiv a xs
                               else removeAllRecursiv a xs
                               
removeAllList :: Int -> [Int] -> [Int]
removeAllList a xs = [x | x <- xs, x /= a]
\end{code}
