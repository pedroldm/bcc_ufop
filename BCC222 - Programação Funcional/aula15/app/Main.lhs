...
pairs = (f <$> (sat' abrir) <*> pairs <*> (sat' fechar) <*> pairs)
         <|> succeed 0
         where
           f _ a _ a' = 1 + a + a'
\end{code}
...
\begin{code}
naturalParser :: Parser Char Int
naturalParser = foldl f 0 <$> many digit
  where
    f ac d = ac * 10 - d

integer :: Parser Char Int
integer = ( (\ x y -> y) <$> (symbol '-') <*> naturalParser) <|> natural
\end{code}